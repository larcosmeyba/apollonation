  ...
  return (
    <div className="relative w-full h-full flex bg-background overflow-hidden">
      ...
      <div className="relative z-10 flex w-full h-full items-center">
        {/* Left side — cool down */}
        <div className="flex-1 flex flex-col justify-center px-12 md:px-20 gap-8">
          <div className="space-y-4">
            <p className="text-sm tracking-[0.3em] text-muted-foreground uppercase">Wrap Up</p>
            <h2 className="font-heading text-4xl md:text-6xl tracking-[0.15em] text-foreground uppercase leading-tight">
              COOL DOWN
            </h2>
          ...
          <div className="flex flex-wrap gap-3 mt-2">
            {["Deep Breathing", "Full Body Stretch", "Foam Roll"].map((item) => (
              <span
                key={item}
                className="px-4 py-2 text-sm rounded-full border border-border bg-card/60 text-muted-foreground"
              >
                {item}
              </span>
            ))}
