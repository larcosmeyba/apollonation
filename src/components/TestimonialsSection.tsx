        ...
        <div className="grid md:grid-cols-3 gap-5 max-w-5xl mx-auto">
          {testimonials.map((testimonial) => (
            <div
              ...
              className="relative p-8 border border-border bg-card/30 hover:border-foreground/15 transition-all duration-500 rounded-xl"
            >
              <Quote className="absolute top-6 right-6 w-8 h-8 text-foreground/5" strokeWidth={1} />
              
              <div className="flex gap-0.5 mb-6">
                {[...Array(testimonial.rating)].map((_, i) => (
                  <Star key={i} size={12} className="text-foreground/40 fill-foreground/40" />
                ))}
              </div>

              <p className="text-foreground/60 mb-8 leading-relaxed font-light text-sm">
                "{testimonial.content}"
              </p>

              <div className="flex items-center gap-4">
                <div className="w-10 h-10 border border-border flex items-center justify-center bg-muted rounded-full">
                  <span className="font-heading text-foreground/60 text-sm tracking-wider">
                    {testimonial.name[0]}
                  </span>
                </div>
                <div>
                  <div className="font-medium text-foreground text-sm">{testimonial.name}</div>
                  <div className="text-[10px] text-muted-foreground font-light">{testimonial.role}</div>
                </div>
              </div>
            </div>
          ))}
