    ...
    <footer className="border-t border-border/50 bg-background/80 backdrop-blur-sm">
      <div className="container mx-auto px-4 py-16">
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12">
            ...
            <div className="flex gap-3">
              {socialLinks.map((social) => (
                <a
                  ...
                  className="w-9 h-9 border border-white/20 flex items-center justify-center text-white/60 hover:border-white/30 hover:text-white transition-all duration-500 rounded-full"
          ...
          <div>
            <h4 className="font-heading text-xs uppercase tracking-[0.2em] text-white mb-6">Experience</h4>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {footerLinks.support.map((link) => (
                <li key={link.label}>
                  {link.isLink ? (
                    <Link to={link.href} className="text-white/60 hover:text-white transition-colors duration-500 text-sm font-light">
                      {link.label}
                    </Link>
                  ) : (
                    <a href={link.href} className="text-white/60 hover:text-white transition-colors duration-500 text-sm font-light">
