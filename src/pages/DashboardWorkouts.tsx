...
const AdminClassPlayerLauncher = ({ classId, title, onClose }: { classId: string; title: string; onClose: () => void }) => {
  const { data, isLoading, error } = useQuery({
    queryKey: ["admin-class-blocks", classId],
    queryFn: async () => {
      const { data: blocks, error: bErr } = await supabase
        .from("admin_class_blocks")
        .select("*, exercise:admin_exercises!admin_class_blocks_exercise_id_fkey(*), alt:admin_exercises!admin_class_blocks_alt_exercise_id_fkey(*)")
        .eq("class_id", classId);
      if (bErr) throw bErr;
  ...
  if (error || !data || data.length === 0) {
    return (
      <div className="fixed inset-0 z-[100] bg-black flex flex-col items-center justify-center text-white gap-4 p-6 text-center">
        ...
        <p className="text-sm text-white/70">An admin needs to add exercises to it before it can be played.</p>
        <button onClick={onClose} className="mt-2 px-4 py-2 rounded-lg bg-white text-black font-semibold">Close</button>
      </div>
    );
  }
              ...
              <div className="flex gap-6 overflow-x-auto pb-2 -mx-4 px-4 scrollbar-hide">
                <Link to="/dashboard/coach/marcos" className="flex flex-col items-center gap-2 flex-shrink-0">
                  <div className="w-24 h-24 rounded-full overflow-hidden border-[3px] border-white shadow-[0_0_25px_rgba(255,255,255,0.25),0_0_50px_rgba(255,255,255,0.1)]">
                    <img loading="lazy" decoding="async" src={marcosAction1} alt="Marcos Leyba" className="w-full h-full object-cover object-[center_15%]" />
                  </div>
            ...
            <>
              {(selectedWorkout as any).mux_playback_id ? (
                ...
                <div className="relative aspect-video w-full overflow-hidden">
                  <img loading="lazy" decoding="async" src={getWorkoutThumbnail(selectedWorkout)!} alt={selectedWorkout.title} className="w-full h-full object-cover" />
                </div>
              ) : null}
