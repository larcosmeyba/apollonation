// Enqueue an On-Demand class for rendering on the EXTERNAL FFmpeg worker.
// This function does NO rendering itself — it builds the ordered clip list
// from the class blocks and hands the job to the worker over HTTP, then
// returns immediately. The worker calls `render-callback` when finished.
...
const RENDER_WORKER_URL = Deno.env.get("RENDER_WORKER_URL"); // e.g. https://apollo-render.up.railway.app
...
// Mux auto-generates a static medium.mp4 rendition for every public asset —
// that gives the worker a plain downloadable URL per clip.
...
Deno.serve(async (req) => {
  ...
  try {
    if (!RENDER_WORKER_URL || !RENDER_WORKER_SECRET) {
      return json({ error: "Render worker is not configured (RENDER_WORKER_URL / RENDER_WORKER_SECRET)" }, 500);
    }

    const authHeader = req.headers.get("Authorization");
    if (!authHeader?.startsWith("Bearer ")) return json({ error: "Unauthorized" }, 401);

    // RLS-scoped client: only an admin can read admin_classes/blocks.
    ...
    const { data: cls, error: clsErr } = await supabase
      .from("admin_classes").select("*").eq("id", class_id).maybeSingle();
    if (clsErr || !cls) return json({ error: "Class not found or forbidden" }, 403);

    const { data: blocks, error: blocksErr } = await supabase
      .from("admin_class_blocks").select("*").eq("class_id", class_id).order("sort_order");
    ...
    // Build the ordered segment list the worker will stitch.
    type Segment =
      | { kind: "exercise"; url: string; isMux: boolean; in: number; out: number; fillSeconds: number }
    ...
    for (const b of blocks as Block[]) {
      ...
      const url = isMux ? muxMp4(ex.mux_playback_id!) : ex.source_video_url;
      if (!url) continue;
    ...
    // Persist the job first so failures are visible in the UI.
    const { data: job, error: jobErr } = await supabase
      ...
      .insert({
        ...
        inputs_json: { title: cls.title, segments },
        created_by: user.id,
      })
      .select()
      .single();
    ...
    // Hand off to the external worker. Fire the request but don't wait for the
    // render — the worker ACKs fast and renders async, calling render-callback.
    const callbackUrl = `${Deno.env.get("SUPABASE_URL")}/functions/v1/render-callback`;
    let workerResp: Response;
    try {
      workerResp = await fetch(`${RENDER_WORKER_URL}/render`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-worker-secret": RENDER_WORKER_SECRET },
        body: JSON.stringify({
          jobId: job.id,
          title: cls.title,
          segments,
